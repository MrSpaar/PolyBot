...
SQLQuery::SQLQuery(Database &db, const std::string_view &sql) {
    int rc = sqlite3_prepare_v2(db.conn, sql.data(), (int) sql.size(), &stmt, nullptr);

    if (rc != SQLITE_OK)
        throw std::runtime_error("Could not prepare statement: " + std::string(sqlite3_errmsg(db.conn)));
}
