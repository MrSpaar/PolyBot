...
auto move_callback(
        Bot &bot, const dpp::guild_member &member, const dpp::channel &voice_channel,
...
void channel_joined_handler(Bot &bot, const dpp::guild_member &member, const dpp::snowflake &category_id) {
    ...
    bot.channel_create(dpp::channel()
            ...
            [&, member, category_id, name](const dpp::confirmation_callback_t &callback) {
                ...
                bot.guild_member_move(
                        voice_channel.id, member.guild_id, member.user_id,
                        move_callback(bot, member, voice_channel, name, category_id)
                );
            }
    );
}
...
void channel_left_handler(Bot &bot, const dpp::voice_state_update_t &event) {
...
void Bot::voiceHandler(const dpp::voice_state_update_t &event) {
    ...
    if (joined == nullptr)
        return channel_left_handler(*this, event);
    ...
    if (!joined->parent_id.empty())
        return channel_joined_handler(*this, member, joined->parent_id);
}
