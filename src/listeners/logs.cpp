...
void Listeners::onGuildMemberAdd(const dpp::guild_member_add_t &event) {
    ...
    Env::SQL << "SELECT welcome_channel, welcome_message, logs_channel, newcomer_role FROM guilds WHERE id = ?", guild_id, std::endl;
    if (!Env::SQL.good())
        return;
...
void Listeners::onGuildMemberRemove(const dpp::guild_member_remove_t &event) {
    ...
    Env::SQL << "SELECT logs_channel FROM guilds WHERE id = ?", guild_id, std::endl;
    if (!Env::SQL.good())
        return;
...
void Listeners::onGuildBanRemove(const dpp::guild_ban_remove_t &event) {
    Env::SQL << "SELECT logs_channel FROM guilds WHERE id = ?", std::to_string(event.unbanning_guild->id), std::endl;
    if (!Env::SQL.good())
        return;
